...
3. 分时操作系统通常采用时间片轮转调度算法。  
4. 两道作业的批处理系统=内存中能存放两条进程。
5. 抢占式短作业优先，新来的进程和当前进程剩余时间比较。
...
1. FCFS：
    ...
    + 不会饥饿
2. SJF:
    + 追求最少的平均等待时间，最少平均周转时间，最少平均带权周转时间。
    + 最短的进程优先得到服务。
    + 既可以用于作业调度，也可以用于进程调度。
    + 默认是非抢占式的，但是也可以有抢占式的版本-最短剩余时间优先算法（SRTN）。
    + 优缺点：
        1. 最短（SRTN比SJF更短）平均等待时间，平均周转时间。
        2. 不公平，对短作业有利，对长作业不利。可能产生饥饿。
        3. 预计运行时间由用户提供，不一定真实。
    + 会导致饥饿
3. HRRN：
    + 综合考虑作业/进程的等待时间和要求服务时间。
    + 每次调度时计算响应比，响应比高的优先调度。
    + 既可以用于作业调度，也可以用于进程调度。
    + 非抢占，当前进程主动放弃才计算响应比。
    + 优缺点：
        1. 综合考虑了等待时间和运行时间。
    ...
    + 不会饥饿
4. RR:
    + 公平的，轮流的为各个进程服务，每个进程一定时间间隔内都可以得到响应。
    + 轮流让各个进程执行一个时间片，如果进程没有执行完，则放回队尾重新排队。
    + 用于进程调度。
    + 属于抢占式算法。由时钟装置发出时钟中断来通知CPU。
    + 优缺点：
        1. 公平，响应快，适用分时操作系统。
        2. 高频率切换有一定开销。
        3. 不区分任务紧急程度。
...
5. 优先级调度算法：
    ...
    + 一般情况下：
        ...
        2. 系统进程优先级高于用户进程
        3. 前台进程优先级高于后台进程
        4. 操作系统更偏好I/O进程（I/O繁忙型进程）
        5. 动态优先级可以从追求公平，提升资源利用率进行考虑。
    + 优缺点:
        1. 优先级区分紧急程度，重要程度，适用于实时操作系统。
        2. 若源源不断地有高优先进程到来，可能导致饥饿。
    + 会饥饿
6. 多级反馈队列调度算法：
    + 算法规则：
        1. 设置多级就绪队列，优先级从高到低，时间片从小到大。
...
#### #1临界资源
一次仅允许一个进程使用的资源成为邻接资源。  
邻接资源的访问分为四步：
1. 进入区。 检查是否可以进入临界区。能进入则设置正在访问临界区标志，阻止其他进程访问。
2. 临界区。 临界资源的那段代码，又称临界段。
3. 退出区。 将正在访问临界区标志清除。
4. 剩余区。 代码的其他部分。
...
进入区和退出区是负责实现互斥的代码段。**
#### #2同步
同步也称为直接制约关系，指为完成某种任务而建立的两个或多个进程，这些进程因为需要在某系位置上协调它们的工作次序而等待，传递消息所产生的制约关系。
#### #3互斥
互斥也称为间接制约关系。
如访问临界区时，已经有进程在访问，那么其他进程必须等待。  
为禁止两个进程同时进入临界区，同步机制应该遵循以下准则：  
1. 空闲让进。 临界区空闲时应允许一个请求访问临界区的进程进入。
2. 忙则等待。 临界区已经被访问时，下一个请求的进程必须等待。
3. 有限等待。 对请求访问的进程，应保证在有限时间内进入临界区（防止饥饿）
4. 让权等待。 当进程不能访问临界区，应立即释放处理器，防止进程忙等待（进程无法继续前进，但是依旧占用处理机）。
...
##### （1）单标志法。
算法思想： 一个进程在访问完临界区后把使用临界区的权限转交给另一个进程。每个进程进入临界区的权限只能被另一个进程赋予。
...
P<sub>1</sub>进程：
  ...
  ```
算法缺点：
1. 两个进程只能交替进入临界区，若一个进程不再进入临界区，另一个进程也无法进入临界区。
2. 违反“空闲让进”

##### （2）双标志先坚查法。
算法思想：设置一个bool型数组flag[]，数组中各个元素用来标记进程想进入临界区的意愿。比如“flag[0]=true”代表0号进程想进入临界区。  
每个进程进入临界区前先检查当前有没有别的进程想进入临界区，如果没有，则吧自身对应标志改为true，之后访问临界区。  
P<sub>0</sub>进程：
```c
while(flag[1]); //查看当前是否有进程想进入临界区
flag[0]=true;  // 没有其他进程在临界区，自己想进入
critical section;  //访问临界区
flag[0]=false;  //访问结束，修改自己为不想进临界区
remainder section;
```
P<sub>1</sub>进程：
```c
while(flag[0]); //查看当前是否有进程想进入临界区
flag[1]=true;  // 没有其他进程在临界区，自己想进入
critical section;  //访问临界区
flag[1]=false;  //访问结束，修改自己为不想进临界区
remainder section;
```
缺点：  
1. 进程具有异步性，如果两个进程都先运行第一行，发现没有进程想进入临界区。
2. 然后先后运行第二行，同时设置自己想进临界区，导致两个进程同时访问临界区。
3. 违反“忙则等待”

##### （3）双标志后检查法。
算法思想： 双标志先检查法的改进。先将自己标志设置为“true”，然后再检查对方标志状态，如果也为true，则等待。
P<sub>0</sub>进程：
```c
flag[0]=true; //先表达自己进入临界区意愿
while(flag[1]);  //等别人访问完临界区
critical section; //访问临界区
flag[0]=false;  //退出区
remainder section;
```
P<sub>1</sub>进程：
```c
flag[1]=true; //先表达自己进入临界区意愿
while(flag[0]);  //等别人访问完临界区
critical section; //访问临界区
flag[1]=false;  //退出区
remainder section;
```
缺点：
1. 进程具有异步性，如果两个同时将自己标志值flag设为true，然后执行while语句时会同时等待。双方“谦让”，谁也不进临界区，导致饥饿。
2. 违背“空闲让进”和“有限等待”
##### （4）Peterson Algorithm。
算法思想： 在双标志后检查法中，再增加一个顺序位turn，实现孔融让梨。
```c
  bool flag[2];
  int turn=0;
```
...
```c
  flag[0]=turn; //表达自己意愿
  turn=1;  //让梨
  while(flag[1] && turn==1);  // 对方想进，并且最后一次自己让梨
...
```c
  flag[1]=turn; //表达自己意愿
  turn=0;  //让梨
  while(flag[0] && turn==0);  // 对方想进，并且最后一次自己让梨
  critical section;  //进入临界区
  flag[1]=false;  //退出区
  remainder section;
```
特点：
1. 遵循了空闲让进，忙则等待，有限等待三个原则。但未实现让权等待。
...
/*如果没有上锁可以进去，然后锁上。开始访问临界区。
运行结束后在while循环外解锁。 */
```
优点： TSL指令是硬件原子操作，上锁和检查是一气呵成的。实现简单，适用于多处理机环境。
缺点： 不满足让权等待，暂时无法进入临界区的进程会占用CPU并循环执行TSL指令，导致忙等。

#### (3)Swap指令
 逻辑上与TSL指令相似，先设置一个共享bool变量lock，初值为false（未上锁）,每个进程再设置一个局部bool变量key，先用swap交换key值和lock值，然后检查key值，如果为false，则进入临界区。
 ```c
 key=true;
 while (key!=true) {
   Swap(&lock,&key);
   //交换key和lock值，如果key仍为true，重复while循环
 }
 //进程进入临界区
 lock=false; //访问临界区结束
 ```
...
用户进程可以通过使用操作系统提供的一对原语来对信号量进行操作，实现进程互斥、进程同步。  
**信号量就是一个变量，可以用一个信号量来表示系统中某种资源的数量。**  
一对原语：wait(S)原语和signal(S)。可以这么理解，wait()和signal()是函数，信号量S是调用传入的参数。  
wait,和signal原语简称P,V操作。可以写成P(S)和V(S)。  
...
void wait (semaphore S){
  S.value--;
  if (S.value<0){  
    block(S.L);  //block原语使进程变为阻塞态。放进等待队列
  }
}
void signal (semaphore S){
  s.value++;
  if(S.value<-0){  //释放资源后仍然有进程在等待队列
    wakeup(S.L);  //Wakeup原语，从等待队列中将一个进程从阻塞态变为就绪态。
  }
}

```
**纪录型信号量，在申请资源时，先对value- -，然后再判断剩余资源数量。所以value会出现负数，表示处于阻塞态的等待队列中依然还有进程。**  

#### #3利用信号量实现进程互斥
```c
semaphore mutex=1;  //临界区可以认为是资源数量只有1的资源。
P1(){
  P(S);
  //进程P1进入临界区
  V(S);
...
#### #4信号量机制实现进程同步
回顾：同步是指进程因为具有异步性，导致两个进程交替推进的次序是不确定的，如果一个进程需求另一个进程的运行结果，那么不同的运行先后顺序，会导致运行结果不同。
```c
semaphore S=0;
//语句y一定要在语句x之后执行
P1(){
...
```
设计信号量S为0，语句y执行前需要先V操作，但是信号量为0，P操作会执行block原语，进程进入阻塞队列。等到P1执行V操作，执行wakeup原语，P2继续执行。

#### #5进程的前驱关系
多层次的同步关系。
**保证进程执行的先后关系，保证代码段的前驱执行后，才会执行这个代码段。**  
